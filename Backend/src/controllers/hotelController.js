...
const getAllHotels = async (req, res) => {
    try {
        ...
        const roomFilters = {};
        if (minPrice) {
            roomFilters['roomDetails.pricePerNight'] = { ...roomFilters['roomDetails.pricePerNight'], $gte: parseInt(minPrice, 10) };
        }
        if (maxPrice) {
            roomFilters['roomDetails.pricePerNight'] = { ...roomFilters['roomDetails.pricePerNight'], $lte: parseInt(maxPrice, 10) };
        }
        if (amenities) {
