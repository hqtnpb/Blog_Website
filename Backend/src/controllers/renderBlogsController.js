  ...
  likeBlog: (req, res) => {
    ...
    Blog.findOneAndUpdate(
      ...
      if (!isLikeByUser) {
        ...
        like.save().then(async (notification) => {
          // Send real-time notification to blog author
          try {
            const liker = await User.findById(user_id).select(
              "personal_info.username"
            );
            const realTimeNotif = createNotification(
              NotificationTypes.BLOG_LIKE,
              "Lượt thích mới",
              `${liker?.personal_info?.username || "Một người dùng"} đã thích bài viết của bạn`,
              {
                blogId: _id,
                userId: user_id,
                username: liker?.personal_info?.username,
              }
            );
            sendNotificationToUser(blog.author, realTimeNotif);
          } catch (notifError) {
